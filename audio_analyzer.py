...
from config import AudioInputMode, AudioConfig
...
class AudioAnalyzer:
    ...
    def __init__(self, 
        ...
        Args:
            ...
            device_name: Device name to use (takes priority over input_mode if found)
    ...
    def start(self) -> bool:
        ...
        from audio_devices import find_device_by_name, get_best_device_for_mode, AudioDeviceType
        ...
        try:
            ...
            if self.device_index is not None:
                ...
                device_info = find_device_by_name(self._device_name)
                if device_info:
                    self.device_index = device_info.index
                    is_loopback = device_info.device_type in (AudioDeviceType.LOOPBACK, AudioDeviceType.MONITOR)
                    self._actual_input_mode = AudioInputMode.LOOPBACK if is_loopback else AudioInputMode.MICROPHONE
            ...
            if self.device_index is None:
                # No explicit device - use mode to find best device
                device_info = get_best_device_for_mode(self.input_mode.value)
                if device_info:
                    self.device_index = device_info.index
                    is_loopback = device_info.device_type in (AudioDeviceType.LOOPBACK, AudioDeviceType.MONITOR)
                    self._actual_input_mode = AudioInputMode.LOOPBACK if is_loopback else AudioInputMode.MICROPHONE
    ...
    def _process_audio(self, audio_data: np.ndarray) -> None:
        ...
        if self._beat_activations:
            ...
            # Use fixed reference (0.1 = moderate audio level)
            normalized_rms = min(1.0, (rms / 0.1) * self._gain)
        ...
        # Calculate signal power (RMS²) for relative normalization
        # This is the reference for all band powers
        signal_power = rms ** 2
        ...
        if len(audio_data) >= self._fft_size:
            ...
            # Calculate mean power per bin for each frequency band
            bass_power, _ = self._get_band_power(fft_data, 20, 250)
            ...
            self._mid_history.append(mid_norm)
            self._high_history.append(high_norm)
    ...
    def _get_waveform_display(self, audio_data: np.ndarray, num_points: int = 100) -> List[float]:
        ...
        for i in range(num_points):
            ...
            if start < len(audio_data):
                ...
                if max_val > 0:
                    # Normalize with fixed reference (0.3 = moderate peak level) and gain
                    normalized = min(1.0, (max_val / 0.3) * self._gain)
        ...
        try:
            ...
            for i in range(num_bands):
                ...
                if high_idx > low_idx:
                    ...
                    # Normalize with fixed reference (0.01 = moderate spectrum energy) and gain
                    normalized = min(1.0, (band_energy / 0.01) * self._gain)
    ...
    def _get_band_power(self, fft_data: np.ndarray, low_freq: float, high_freq: float) -> tuple[float, int]:
        """
        Get mean power per bin in a frequency band from FFT data.
        
        Returns:
            tuple: (mean_power_per_bin, num_bins)
        """
        ...
        num_bins = high_idx - low_idx
        # Mean of squared magnitudes (average power per bin)
        mean_power = float(np.mean(fft_data[low_idx:high_idx] ** 2))
        return mean_power, num_bins
...
def create_audio_analyzer(
    ...
    return AudioAnalyzer(device_index=device_index, device_name=device_name, input_mode=input_mode)
